   ...
   <details>
    ...
    4. Add `New Run Script Phase` on `TARGETS` > `Build Phase`
        ...
        case ${CONFIGURATION} in
          Development*)
            cp -r "${SRCROOT}/${PROJECT_NAME}/Firebase/Development/GoogleService-Info.plist" "${BUILT_PRODUCTS_DIR}/${PRODUCT_NAME}.app/GoogleService-Info.plist" ;;
          Production*)
            cp -r "${SRCROOT}/${PROJECT_NAME}/Firebase/Production/GoogleService-Info.plist" "${BUILT_PRODUCTS_DIR}/${PRODUCT_NAME}.app/GoogleService-Info.plist" ;;
          *) ;;
        esac
        ```
       > 💡 Rename this script as `Firebase Environment Configurations`  
